    ...
    try {
      ...
      if (game) {
        ...
        io.to(gameId).emit('gameEnd', { 
          result: result === 'resignation' ? `${winnerColor} wins by resignation` : result 
        });
...
io.of('/spectate').on('connection', (socket) => {
...
redisSub.subscribe('gameUpdate', (message) => {
  try {
    ...
    io.of('/spectate').to(`spectate:${data.gameId}`).emit('gameUpdate', data.state);
  } catch (error) {
