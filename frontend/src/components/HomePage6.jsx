...
const sendGameResult = async (gameData) => {
  try {
    const response = await fetch('http://localhost:5000/game-result', {
      method: 'POST',
      headers: {
...
const [gameStartTime, setGameStartTime] = useState(null);
  ...
  if (playerColor === 'w') {
    ...
    socket.on('gameEnd', ({ result, winner, reason }) => {
      ...
      setIsPlayerTurn(false);
       if (!isSpectating && gameStartTime) {
    const gameResult = {
      playerName: playerName,
      opponentName: opponentName,
      winner: winner, // This will come from backend
      reason: reason, // This will come from backend (checkmate, draw, resign, timeout)
  ...
  const resign = () => {
    ...
    const gameResult = {
      playerName: playerName,
      opponentName: opponentName,
      winner: opponentName, // Opponent wins when current player resigns
      reason: 'resign',
      gameStartTime: gameStartTime,
      gameEndTime: new Date().toISOString()
    };
