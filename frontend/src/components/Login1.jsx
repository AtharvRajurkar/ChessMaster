...
//       {user ? (
//         <>
//          {/* <AuthSuccess user={user} onLogout={handleLogout} /> */}
//             <ChessGame />
//             </>
//       ) : (
//         <AuthForm 
...
//         <h1 className="text-2xl font-bold text-white mb-2">Welcome, {user.playerName}!</h1>
...
//         localStorage.setItem('playerName', data.user.playerName);
...
import React, { useState } from 'react';
import ChessGame from './HomePage5';
import { Eye, EyeOff, User, Mail, Lock, Crown } from 'lucide-react';
...
export default function ChessAuth() {
  ...
  // Fixed handleAuth function with cleaner parameter handling
  const handleAuth = async (authData) => {
    ...
    try {
      ...
      console.log('Request sent:', { type: authData.type, body: requestBody });
      ...
      if (response.ok) {
        ...
        setUser(data.user);
        setError('');
        // Store player info in localStorage (note: localStorage isn't available in artifacts)
        ...
        <>
          ...
          <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-8 w-full max-w-md border border-white/20 shadow-2xl text-center">
            ...
            <div className="space-y-2 text-sm text-gray-300 mb-6">
              ...
              <p>Rating: {user.playerRating}</p>
              <p>Games: {user.totalGames}</p>
            </div>
            <button
